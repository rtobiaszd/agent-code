...
exports.CONFIG = {
    ...
    MODEL_PLANNER_FALLBACK: process.env.MODEL_PLANNER_FALLBACK || process.env.MODEL_PLANNER || 'qwen2.5-coder:7b',
    MODEL_EXECUTOR: process.env.MODEL_EXECUTOR || 'qwen2.5-coder:7b',
    MODEL_EXECUTOR_FALLBACK: process.env.MODEL_EXECUTOR_FALLBACK || process.env.MODEL_EXECUTOR || 'qwen2.5-coder:7b',
    MODEL_REVIEWER: process.env.MODEL_REVIEWER || 'qwen2.5-coder:7b',
    MODEL_REVIEWER_FALLBACK: process.env.MODEL_REVIEWER_FALLBACK || process.env.MODEL_REVIEWER || 'qwen2.5-coder:7b',
    MODEL_FIXER: process.env.MODEL_FIXER || 'qwen2.5-coder:7b',
    MODEL_FIXER_FALLBACK: process.env.MODEL_FIXER_FALLBACK || process.env.MODEL_FIXER || 'qwen2.5-coder:7b',
    MODEL_JSON_REPAIR: process.env.MODEL_JSON_REPAIR || process.env.MODEL_FIXER || 'qwen2.5-coder:7b',
    MODEL_TIMEOUT_MS: Number(process.env.MODEL_TIMEOUT_MS || 1000 * 60 * 10),
    MODEL_RETRY_ATTEMPTS: Number(process.env.MODEL_RETRY_ATTEMPTS || 2),
    MODEL_RETRY_BACKOFF_MS: Number(process.env.MODEL_RETRY_BACKOFF_MS || 800),
    MODEL_RETRY_MAX_BACKOFF_MS: Number(process.env.MODEL_RETRY_MAX_BACKOFF_MS || 10000),
    ...
    REPLAN_INTERVAL_CYCLES: Number(process.env.REPLAN_INTERVAL_CYCLES || 3),
    CRITICAL_FAILURE_REPLAN_THRESHOLD: Number(process.env.CRITICAL_FAILURE_REPLAN_THRESHOLD || 2),
    ...
    AUTO_INSTALL_MISSING_PACKAGES: String(process.env.AUTO_INSTALL_MISSING_PACKAGES || 'true').toLowerCase() === 'true',
    ALLOW_DEV_DEP_INSTALLS: String(process.env.ALLOW_DEV_DEP_INSTALLS || 'true').toLowerCase() === 'true',
    MAX_AUTO_INSTALLS_PER_ITERATION: Number(process.env.MAX_AUTO_INSTALLS_PER_ITERATION || 3),
    NPM_CLIENT: process.env.NPM_CLIENT || '',
    REPO_STABILIZATION_MODE: String(process.env.REPO_STABILIZATION_MODE || 'true').toLowerCase() === 'true',
    ...
    STABILIZATION_ALLOWED_NEW_FILES: ['eslint.config.js', 'eslint.config.cjs', 'eslint.config.mjs', 'jest.config.js', 'jest.config.cjs', 'jest.config.ts', 'tsconfig.json', 'tsconfig.build.json', 'tsconfig.test.json', 'vitest.config.ts', 'src/test/setup.ts', 'test/setup.ts'],
    FORBIDDEN_TECH_KEYWORDS: ['firebase admin sdk', 'mongo', 'mongodb', 'mongoose', 'fastify', 'koa', 'hapi', 'django', 'flask', 'rails', 'laravel', 'supabase', 'graphql']
};
function repoPath(...parts) {
    return path_1.default.join(exports.CONFIG.REPO_PATH, ...parts);
}
